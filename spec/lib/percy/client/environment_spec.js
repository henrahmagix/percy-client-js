    ...
    describe('no known CI environment', function () {
        ...
        describe('#repo', function () {
            it('returns the current local repo name', function () {
                expect(Percy.Client.Environment.repo).toBe('percy/percy-client');
            });
            ...
            it('handles git ssh urls', function () {
                spyOnProperty(Percy.Client.Environment, '_get_origin_url', 'get')
                    .and.returnValue('git@github.com:org-name/repo-name.git');
                expect(Percy.Client.Environment._get_origin_url.calls.count()).toBe(1);
                expect(Percy.Client.Environment.repo).toBe('org-name/repo-name');

                spyOnProperty(Percy.Client.Environment, '_get_origin_url', 'get')
                    .and.returnValue('git@github.com:org-name/repo-name.org.git');
                expect(Percy.Client.Environment._get_origin_url.calls.count()).toBe(1);
                expect(Percy.Client.Environment.repo).toBe('org-name/repo-name.org');

                spyOnProperty(Percy.Client.Environment, '_get_origin_url', 'get')
                    .and.returnValue('git@custom-local-hostname:org-name/repo-name.org');
                expect(Percy.Client.Environment._get_origin_url.calls.count()).toBe(1);
                expect(Percy.Client.Environment.repo).toBe('org-name/repo-name.org');
            });
            it('handles git https urls', function () {
                spyOnProperty(Percy.Client.Environment, '_get_origin_url', 'get')
                    .and.returnValue('https://github.com/org-name/repo-name.git');
                expect(Percy.Client.Environment._get_origin_url.calls.count()).toBe(1);
                expect(Percy.Client.Environment.repo).toBe('org-name/repo-name');

                spyOnProperty(Percy.Client.Environment, '_get_origin_url', 'get')
                    .and.returnValue('https://github.com/org-name/repo-name.org.git');
                expect(Percy.Client.Environment._get_origin_url.calls.count()).toBe(1);
                expect(Percy.Client.Environment.repo).toBe('org-name/repo-name.org');

                spyOnProperty(Percy.Client.Environment, '_get_origin_url', 'get')
                    .and.returnValue("https://github.com/org-name/repo-name.org\n");
                expect(Percy.Client.Environment._get_origin_url.calls.count()).toBe(1);
                expect(Percy.Client.Environment.repo).toBe('org-name/repo-name.org');
            });
    ...
    describe('local git repo methods', function () {
        describe('#commit', function () {
            ...
            it('returns only branch if commit data cannot be found', function () {
                spyOnProperty(Percy.Client.Environment, '_raw_commit_output', 'get')
